...
use rand::Rng;
use wasm_spatial_core::{Octree, parse_las_header_core, parse_laz_points_core};
...
fn build_laz(n: usize) -> Vec<u8> {
    ...
    let laz_items = laz::LazItemRecordBuilder::new()
        .add_item(laz::LazItemType::Point10).build();
    ...
    let vlr = laz::LazVlr::from_laz_items(
        laz::LazItemRecordBuilder::new().add_item(laz::LazItemType::Point10).build());
    let vd = { let mut b = Cursor::new(Vec::new()); vlr.write_to(&mut b).unwrap(); b.into_inner() };
    let vt = (2 + 16 + 2 + 2 + 32) + vd.len();
    let hs: u32 = 230;
    let po = hs + vt as u32;
    let mut buf = vec![0u8; hs as usize];
    buf[0..4].copy_from_slice(b"LASF"); buf[24] = 1; buf[25] = 2;
...
fn test_50m_laz_header() {
    ...
    println!("✅ 50M LAZ header: {} points, v{}.{}", hdr.num_points(), hdr.version_major(), hdr.version_minor());
}
...
fn test_50m_laz_full_pipeline() {
    ...
    let laz = build_laz(n);
    let gen = t.elapsed();
    println!("  [Gen]      {:.1} MB LAZ in {:?}", laz.len() as f64 / 1e6, gen);
    ...
    println!("  [Octree]   {} nodes, {} leaves", octree.node_count(), octree.leaf_count());
    println!("  [Tileset]  {} tiles, {:.1} MB", tileset.tile_count(), tileset.total_bytes() as f64 / 1e6);
